...
const SITE_URL = 'https://shahbaz-khan-portfolio-flax.vercel.app/';
const navItems = [
...
function Avatar3D({ reducedMotion, onLoadError }) {
 const group = useRef();
 ...
 const pointerDown = e => {
   drag.current = { active:true, x:e.clientX, pointerId:e.pointerId };
   e.stopPropagation();
   e.target.setPointerCapture?.(e.pointerId);
 };
 const pointerMove = e => {
   if (!drag.current.active || drag.current.pointerId !== e.pointerId || !group.current) return;
 ...
 const endDrag = e => {
   if (drag.current.pointerId === e.pointerId) {
     ...
     drag.current = { active:false, x:0, pointerId:null };
   }
 };

 return <group ref={group} position={[0,-0.25,0]} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={endDrag} onPointerCancel={endDrag}>
   <Float speed={reducedMotion ? 0 : 1.05} rotationIntensity={reducedMotion ? 0 : 0.025} floatIntensity={reducedMotion ? 0 : 0.045}>
    {texture && <>
      <mesh position={[0,0,-0.18]} scale={[1.025,1.025,1]}>
       <planeGeometry args={[4.0,5.33]}/>
      ...
      <mesh position={[0,0,-0.06]} scale={[0.985,0.985,1]}>
       <planeGeometry args={[4.0,5.33]}/>
       <meshBasicMaterial map={texture} transparent opacity={0.08} color="#8b5cf6" depthWrite={false} side={THREE.DoubleSide}/>
      </mesh>
    </>}
   </Float>
   <Torus args={[2.18,0.012,20,160]} rotation={[Math.PI/2,0,0]} position={[0,-0.22,-0.38]}>
    <meshBasicMaterial color="#18d7ff" transparent opacity={0.68}/>
   </Torus>
   <Torus args={[2.5,0.009,20,160]} rotation={[Math.PI/2,0.38,0]} position={[0,-0.22,-0.35]}>
    <meshBasicMaterial color="#8b5cf6" transparent opacity={0.45}/>
   </Torus>
   <mesh position={[0,-2.62,-0.4]} rotation={[-Math.PI/2,0,0]}>
    <circleGeometry args={[1.7,64]}/>
    <meshBasicMaterial color="#061321" transparent opacity={0.78}/>
   </mesh>
 </group>;
}
...
function Scene({ reducedMotion, onLoadError }) {
 ...
 >
  ...
  <pointLight position={[-4,2,3]} intensity={10} color="#18d7ff" distance={9}/>
  <pointLight position={[4,1,2]} intensity={8} color="#8b5cf6" distance={8}/>
  <Stars radius={28} depth={16} count={reducedMotion ? 350 : 650} factor={1.15} saturation={0} fade speed={reducedMotion ? 0 : 0.25}/>
  {!reducedMotion && <Sparkles count={70} scale={[8,7,5]} size={1.4} speed={0.25} color="#77dcff"/>}
  <Avatar3D reducedMotion={reducedMotion} onLoadError={onLoadError}/>
 </Canvas>;
}
...
function WebGLHero() {
 ...
 >
   ...
   {imageError && <div className="webgl-fallback"><strong>Profile image could not be loaded.</strong><span>Using the accessible portfolio content instead.</span></div>}
 </div>;
}
...
export default function Home() {
 ...
 const personSchema = {
   ...
   jobTitle:'IT Operations Executive / Vibe Coder / AI Integrator',
   url:SITE_URL,
   email:'mailto:skyzai2009@gmail.com',
   address:{'@type':'PostalAddress',addressLocality:'Hyderabad',addressCountry:'PK'},
   sameAs:['https://www.linkedin.com/in/muhammd-shahbaz-khan-06429a377/','https://github.com/Skyzai4u']
 };

 return <>
 <Head>
   <title>Shahbaz Khan | IT Operations • Vibe Coder • AI Integrator</title>
   <meta name="description" content="Muhammad Shahbaz Khan: IT Operations, retail technology, full-stack development, AI integration, automation and business systems."/>
   <meta name="author" content="Muhammad Shahbaz Khan"/>
   <meta name="robots" content="index,follow"/>
   <meta name="theme-color" content="#05070d"/>
   <link rel="canonical" href={SITE_URL}/>
   <meta property="og:type" content="website"/>
   <meta property="og:title" content="Shahbaz Khan | IT Operations • Vibe Coder • AI Integrator"/>
   <meta property="og:description" content="IT Operations, retail technology, full-stack development, AI integration and automation."/>
   <meta property="og:url" content={SITE_URL}/>
   <meta property="og:image" content={SITE_URL + 'og-profile.svg'}/>
   <meta name="twitter:card" content="summary_large_image"/>
   <meta name="twitter:title" content="Shahbaz Khan | IT Operations • Vibe Coder • AI Integrator"/>
   <meta name="twitter:description" content="IT Operations, full-stack development, AI integration and automation."/>
   <meta name="twitter:image" content={SITE_URL + 'og-profile.svg'}/>
   <script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(personSchema)}}/>
 </Head>

 <nav className="nav">
  <div className="wrap nav-inner">
   <a className="brand" href="#" aria-label="Shahbaz Khan home"><span className="brandmark">SK</span><span>Shahbaz Khan</span></a>
   <button className="menu-toggle" aria-expanded={menuOpen} aria-controls="site-navigation" onClick={() => setMenuOpen(v => !v)}>{menuOpen ? 'Close' : 'Menu'}</button>
   <div id="site-navigation" className={menuOpen ? 'links open' : 'links'}>
 ...
 <main className="wrap">
  <section className="hero">
   <div className="hero-copy">
    <div className="eyebrow">IT × AI × AUTOMATION × 3D</div>
    <h1>Hi, I’m<br/><span className="gradient">Shahbaz Khan.</span></h1>
    <p>Technology professional combining real-world IT operations, retail systems, software development, AI integration, automation and business problem solving.</p>
    <div className="actions"><a className="btn primary" href="#work">Explore my work →</a><a className="btn" href="#contact">Get in touch</a></div>
    <div className="hero-mini"><span>IT Operations</span><span>Full-Stack</span><span>AI Integration</span><span>Vibe Coder</span></div>
   </div>
   <div className="hero3d">
    <div className="scene-label">3D / OFFICIAL PROFILE / SHAHBAZ KHAN</div>
    <WebGLHero/>
    <div className="drag">POINTER • DRAG • KEYBOARD</div>
    <div className="avatar-badge"><b>Shahbaz Khan</b><small>IT Operations • Vibe Coder</small></div>
   </div>
  </section>

  <div className="stats">
   <div className="stat"><b>95%</b><span>Tickets resolved within SLA</span></div>
   <div className="stat"><b>50+</b><span>Regular retail stores</span></div>
   <div className="stat"><b>PKR 12M+</b><span>Annual turnover managed</span></div>
   <div className="stat"><b>3+ Days → Hours</b><span>Damage Note Sheet OCR workflow</span></div>
  </div>
  ...
  <section id="education"><div className="section-head"><div><div className="eyebrow">05 / EDUCATION & LANGUAGES</div><h2>Academic foundation.</h2></div><p>Education and language information retained from the supplied resume.</p></div><div className="grid2"><div className="card"><div className="num">EDU</div><h3>Master of Economics</h3><p>University of Sindh, Jamshoro • 2019</p><h3 className="subheading">Bachelor of Arts</h3><p>University of Sindh, Jamshoro</p></div><div className="card"><div className="num">LANG</div><h3>Urdu</h3><p>Native</p><h3 className="subheading">English</h3><p>Professional</p></div></div></section>

  <section id="contact"><div className="contact"><div><div className="eyebrow">06 / CONTACT</div><h2>Let’s build something useful.</h2><p>Open to technology, automation, AI integration, product and IT opportunities.</p></div><div className="card contact-card"><p><b>Hyderabad, Pakistan</b></p><p><a href="mailto:skyzai2009@gmail.com">skyzai2009@gmail.com</a></p><p><a href="https://www.linkedin.com/in/muhammd-shahbaz-khan-06429a377/" target="_blank" rel="noreferrer">LinkedIn ↗</a></p><p><a href="https://github.com/Skyzai4u" target="_blank" rel="noreferrer">GitHub ↗</a></p></div></div></section>
